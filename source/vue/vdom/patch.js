...
export function render(vnode, container) { // 让虚拟节点 渲染重真实节点
...
function createElm(vnode) {
    ...
    if(typeof tag === 'string'){
        ...
        children.forEach(child => { // child是虚拟节点
            return render(child, vnode.el) // 递归渲染当前还在节点
        })
    }else {
        // 文本
...
function updateProperties(vnode, oldProps ={}) {
    let newProps = vnode.props || {}; // 获取当前节点节点中的属性
